...
> **所有 21 组实验的索引与逐项解释见 [[visreg_all_attempts]]**。本页侧重计算差异与理论推导。
...
## 十·五、换赛道尝试：跨模态「相对目标」与作用位置（第四阶段）

第十节的结论是「绝对目标」（把单塔推向 N(0,I)）与下游无因果。本阶段换成
**相对目标** —— 约束图/文两塔的相对关系（即 modality gap），理论上与检索有明确
因果链。实现 `CrossModalMatch`（两塔投到**同一组**随机方向），两种模式：

- **pair**：不排序，逐样本比投影值，配对身份完整保留。`mean_k((v−t)·w_k)²` 在 w
  各向同性时期望 = `‖v−t‖²/D`，即逐对 MSE 的随机投影估计；K<D 相当于「只要求 K 个
  随机子空间对齐、其余方向自由」，比直接 MSE 温和。
- **dist**：sorted shape + std 对齐（即"两塔投影分布形状相等"）。

权重经梯度匹配标定到占比 1e-2（pair w=2.3、dist w=7.8），避免重蹈第十节落进无效区。

### 10.5.1 结果：全部有害，dist 完全崩溃

| run | 配置 | COCO i2t | IN-1k top1 |
|---|---|---|---|
| **E（最优基线）** | cls target，双塔正则 | **24.06** | **23.26** |
| img_only | cls，仅视觉塔正则 | 23.90 | 23.01 |
| clip_base | **clip** target，双塔正则 | 23.08 | 23.26 |
| xm_pair | clip + 逐对对齐 w=2.3 | **19.90** | 22.15 |
| xm_dist | clip + 分布对齐 w=7.8 | **0.02** | 0.11 |

### 10.5.2 dist 模式：置换不变 ⇒ 退化解（预测并验证）

`sort` 是置换不变的，丢弃样本身份，而配对关系恰恰编码在身份里。上线前的单测已量化：

```
文本随机打乱后 loss 变化：dist 模式 < 1e-8（无配对信息）
                          SigLIP    ~1e-3（对配对高度敏感）
两组完全独立的随机高斯：dist loss ≈ 0.019（已接近最优），而检索 R@1 = 1/N
```

即存在平凡退化解：**两塔各自独立地把输出推成 N(0,I)，loss 趋零而模型什么对应都没学到。**

实测证实且比预期更彻底：COCO R@1 = **0.02%**（低于随机），`Contrastive_loss` 全程
卡在 **9.32 不动**（正常应降到 ~5）—— 对比学习完全没有进行。即便只作辅助项、
权重按梯度占比 1e-2 标定，w=7.8 已足以让模型优先满足"分布形状相等"这个容易的目标
而彻底压住对比损失。

⇒ **分布匹配（边缘统计） ≠ 样本级对齐（联合）。前者永远不能替代对比损失。**

### 10.5.3 pair 模式：有配对信号，但与对比学习目标冲突

pair 保留了配对身份（单测：打乱后 2.557→2.607/2.492，明显变化），是本项目里第一个
「保留配对的相对目标」，期望最高。结果 COCO 19.90，比同 target 的 clip_base 又低
3.2pt，且逐 epoch 在 ep7 见顶后持续下滑（19.90→19.32→18.28）——典型过度约束。

**先更正一处设计时的误判**：当初以为「取 K<D 相当于只约束 K 个随机子空间、其余
方向自由，比直接 MSE 温和」。事后验证是错的 —— `mean_k((v−t)·w_k)²` 无论 K 多小
都是 `‖v−t‖²/D` 的**无偏**估计（K=16 偏差 +0.31%、K=256 偏差 +0.08%），K 只影响
方差不影响期望，且方向每 step 重采。**pair 模式就是完整的逐对 MSE**，没有任何
"温和"可言。

失败机制（实测追踪 1200 步）：MSE 式对齐把两个模态的特征云**无差别地叠在一起**——
...
pos 在涨，但**没有任何机制压低 neg**。一旦两塔特征云重叠，图特征 `v_i` 不仅接近
正确文本 `t_i`，也接近所有其他 `t_j` —— 而检索需要的正是把 `t_i` 从 `t_j` 里挑出来。
梯度层面也直接对抗：`cos(∇SigLIP, ∇pair) = −0.0087`。

**更深一层**：与仓库 mgap 系列旧发现「gap 不是越小越好」一致。modality gap 里混着
两种东西 —— **无用的系统性偏移**（两模态质心错位，消掉可能有益）和**有用的模态
特异结构**（图像编码视觉细节、文本编码语义抽象，消掉就是毁信息）。MSE 式对齐不
区分两者、一律压平。而 `ModalityGapLoss`（只对齐均值）之所以能作温和辅助项，恰恰
因为它**只碰第一种**。所以「pair 是 ModalityGapLoss 的高阶推广、因此更有道理」
这个推理是**反的** —— 一阶正是它的优点，不是缺陷。

### 10.5.4 正面发现：正则的作用位置值 ~1pt（cls ≫ clip）

| target | 维度 | 语义 | COCO i2t | IN-1k |
|---|---|---|---|---|
| **cls** | 768 | backbone CLS raw token | **24.06** | 23.26 |
| clip | 1024 | CLIP embedding（对比损失直接在用）| 23.08 | 23.26 |

**差 1.0pt COCO**（IN 持平）。这是本项目除 SIGReg→VISReg 之外，**第二个能产生
~1pt 差异的变量**，且方向明确：正则应作用在 **backbone raw token**，不要放在
CLIP embedding 上。

解释：cls 更靠前、离对比损失更远，正则与主损失的冲突小；clip embedding 正是对比
损失直接使用的表征，在那里加约束等于抢地盘。这也解释了 xm_pair/xm_dist 的失败被
放大 —— 它们因「两塔须同维」被迫在 clip 空间做，本就选了更差的位置，再叠加一个
冲突的目标。

### 10.5.5 附带：text 侧正则贡献 ≈ 0

现行配方 **text tower 一直被正则着**（`CLIPLeJEPA` 同时产出 image_proj/text_proj，
loss 对两者求和）。把它整个去掉（`--reg-sides img`）：COCO 24.06→23.90、
IN 23.26→23.01，均在噪声内。

⇒ VISReg 的 +1.2pt 全部来自视觉塔一侧的形式差异，进一步支持「稳定性而非分布约束」
的归因。
...
**两个真正有效的变量**（本项目共 20+ 组实验里唯二能产生 ~1pt 的）：

| 变量 | 差异 | 增益 |
|---|---|---|
| 正则形式 | SIGReg → VISReg | +1.22 COCO / +2.03 IN |
| **正则位置** | clip embedding → **cls raw token** | **+1.0 COCO**（见 10.5.4）|

其余全部无效：强度（4 个数量级）、目标形状（单峰 vs 混合高斯）、方向选择
（随机 vs top-K / 正交化）、作用侧（双塔 vs 仅视觉）、跨模态相对目标
（逐对对齐 / 分布对齐）。
...
- **工程建议**：CLIP 训练用 VISReg 替代 SIGReg，采用 **scale+shape 等权 + 去 center +
  target=cls**，weight 用梯度匹配标定（~1.83e-4）。**不必再扫强度、不必换目标分布、
  不必做方向工程、不必加跨模态对齐项** —— 这些路本页已全部实测排除。
- **不要尝试用分布匹配替代对比损失**：sorted shape/scale 相等是置换不变的，
  不含配对信息，存在「两塔各自推成 N(0,I)」的退化解。实测 COCO R@1 掉到 0.02%
  且对比损失全程不降（10.5.2）。
