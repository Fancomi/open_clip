...
原因：**逐对 MSE 式对齐与对比学习的目标结构冲突**。对比损失要的是「配对相似 +
非配对不相似」，依赖 batch 内的**相对**关系；pair 对齐要求 `v_i ≈ t_i` 的**绝对**
接近，把两塔特征往一起压，压缩了区分不同样本的可用空间 —— modality gap 变小了，
判别性也变差了。与 mgap 系列早期「gap 不是越小越好」的发现一致。
