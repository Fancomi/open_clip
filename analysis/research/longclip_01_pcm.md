...
### 4.2 `pcm_weight` ≤ 0.5 显著优于 1.0；0.3 与 0.5 不可分辨
...
按 §5.8 的 2σ 判据（COCO i2t 0.75 / Urban i2t 1.92）逐对判定：

| 对比 | COCO i2t | Urban i2t |
|---|---|---|
| 0.5 vs 1.0 | +3.62 ✅ 显著 | +8.80 ✅ 显著 |
| **0.3 vs 0.5** | **+0.52 ❌ 噪声内** | **+1.60 ❌ 噪声内** |

所以能说的是：**w ≤ 0.5 明显优于原论文的 w = 1.0**（两个轴都显著）。
**不能说 0.3 优于 0.5** —— 两个口径的差异都落在噪声内。

机制推测：短分支的 PCA 秩约束（32/1024）本身是对完整 embedding 的强扭曲，
w=1.0 时它与主分支正面冲突；w 小的时候它退化成一个**正则项** ——
足够引导短模板能力，又不足以破坏表示。

**下界未定**：`w=0.1` / `w=0.2` 正在跑（§6.2）。若它们与 0.3 也不可分辨，
则结论是「w ∈ [0.1, 0.5] 是一个平台，任选即可」，而非存在单一最优点。
...
**噪声地板已量出（§5.8）**：k-NN backbone 的 2σ = 0.78，而这里的差值是 0.55 —— 
**在噪声内**，所以准确说法是「**持平，未见提升**」，不是「变差」。

对比 `gemma_dense` 的 −5.95（远超 2σ）：那才是显著退化。
...
- **`w=0.1` / `w=0.2` 探底**（`PCM_WEIGHT=0.1 bash scripts/train/visreg.sh pcm`）。
  §4.2 显示 0.3 与 0.5 不可分辨，需要更小的 w 才能判断是「有最优点」还是「平台」。
