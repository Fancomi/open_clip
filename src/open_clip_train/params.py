...
def parse_args(args):
    ...
    parser.add_argument(
        ...
        help=(
            ...
            'scale=(0.9,1.0) 下实测丢框率（clip_train_region.tsv 的 1/100 代表性切片 = 2 万图，'
            '18.71 候选框/图）：thr=0 **39.92%%**、0.8 1.37%%、0.6 0.23%%。'
            '⚠️ 旧 help 里那组 26.2%% / 6.9%% / 1.8%% 测在 k24 表**前 3000 行**上，'
            '而那一段每图只有 8.29 个候选框（>K 的行 23.9%% vs 全表 77.5%%）—— **不具代表性，已作废**。'
            '⚠️⚠️ thr=0 这一档还伴随 **53.95%% 的短语配对错误**（保留框中占比），见 '
            '--region-crop-fix-align；引用 thr=0 的臂必须说清 fix_align 开没开。'
        )
    )
    parser.add_argument(
