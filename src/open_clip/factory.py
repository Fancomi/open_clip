...
def create_loss(args):
    ...
    if args.distill:
        ...
        return SIGRegContrastiveLoss(
            ...
            region_weight=getattr(args, 'region_weight', 0.0),
            region_gather=getattr(args, 'region_gather', 'local'),
            region_cc_weight=getattr(args, 'region_cc_weight', 0.0),
        )
        ...
        weights = None
        if getattr(args, 'teacher_weights', None):
            weights = [float(w) for w in args.teacher_weights.split(',')]
        return MultiTeacherLoss(
            n_teachers=args._n_teachers,
            weights=weights,
            rank=args.rank,
            world_size=args.world_size,
            dist_impl=getattr(args, 'loss_dist_impl', None),
